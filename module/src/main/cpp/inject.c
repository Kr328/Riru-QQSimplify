...
void load_and_invoke_dex(JNIEnv *env, const char *dex, const char *clazz, const char *method, const char *argument) {
    ...
    jobject   objectDexClassLoader       = (*env)->NewGlobalRef(env, (*env)->NewObject(env ,classDexClassLoader, methodDexClassLoaderInit, stringDexPath, stringOdexPath, NULL, objectSystemClassLoader));

    // get loaded dex inject method
    jmethodID methodFindClass            = (*env)->GetMethodID(env ,classDexClassLoader, "loadClass", "(Ljava/lang/String;)Ljava/lang/Class;");
    jstring   stringInjectClassName      = (*env)->NewStringUTF(env ,clazz);
    jclass    loadedClass                = (jclass)(*env)->CallObjectMethod(env ,objectDexClassLoader, methodFindClass, stringInjectClassName);

    // find method
    jmethodID loadedMethod               = (*env)->GetStaticMethodID(env ,loadedClass , method, DEFAULT_INJECT_SIGNATURE);
